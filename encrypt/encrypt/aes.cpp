...
void AES::test(uint_8* pblock)
{
	...
	for (int i = 0;i < 4;i++)
	{
		...
		for (int j = 0;j < 4;j++)
		{
			tempX = pUchar_256[pblock[xtable_2[i][j]]];
			temp += tempX << (8 * j);
		}
		temp ^= pUint_44[i];
...
void AES::aes_ctr_func(uint_8* pblock, int cnt)
{
	uint_32 temp;
	uint_32 temp_arr[4]{};
	for (int i = 0;i < 4;i++)
	{
		...
		for (int j = 0;j < 4;j++)
		{
			if (j == 0)
			{
				temp ^= pUint_256_1[pblock[xtable_1[i][j]]];
			}
			if (j == 1)
			{
				temp ^= pUint_256_2[pblock[xtable_1[i][j]]];
			}
			if (j == 2)
			{
				temp ^= pUint_256_3[pblock[xtable_1[i][j]]];
			}
			if (j == 3)
			{
				temp ^= pUint_256_4[pblock[xtable_1[i][j]]];
			}
		}
	...
	for (int i = 0;i < 4;i++)
	{
		...
		for (int j = 0;j < 4;j++)
		{
			if (j == 0)
			{
				temp ^= pUint_256_1[pnew_block[xtable_2[i][j]]];
			}
			if (j == 1)
			{
				temp ^= pUint_256_2[pnew_block[xtable_2[i][j]]];
			}
			if (j == 2)
			{
				temp ^= pUint_256_3[pnew_block[xtable_2[i][j]]];
			}
			if (j == 3)
			{
				temp ^= pUint_256_4[pnew_block[xtable_2[i][j]]];
			}
		}
...
void AES::aes(uint_8* pblock)
{
	uint_32 temp;
	for (int i = 0;i < 4;i++)
	{
		temp = *(uint_32*)(pblock + 4 * i) ^ pUint_44[4 * 0xA + i];
		*(uint_32*)(pblock + 4 * i) = temp;
	}

	for (int i = 5;i > 1;i--)
	{
		aes_ctr_func(pblock, i);
	}
	...
	for (int i = 0;i < 4;i++)
	{
		...
		for (int j = 0;j < 4;j++)
		{
			tempX = pUchar_256[pnew_block[xtable_2[i][j]]];
			temp += tempX << (8 * j);
		}
		temp ^= pUint_44[i];
		*(uint_32*)(pblock + 4 * i) = temp;
