#include<stdio.h>
#include<string>
using namespace std;

typedef  unsigned int uint_32;
typedef  unsigned char uint_8;

class encryption {
public:
	const char* pUint_44 = "";
	const char* pUint_256_1 = "";
	const char* pUint_256_2 = "";
	const char* pUint_256_3 = "";
	const char* pUint_256_4 = "";
	const char* pUchar_256 = "";
	uint_8 xtable_1[16] = { 0,13,10,7,
							4,1,14,11,
							8,5,2,15,
							12,9,6,3 };
	uint_8 xtable_2[16] = {12,1,6,11,
							8,13,2,7,
							4,9,14,3,
							0,5,10,15};
public:
	void Encript16ByteEmxl(uint_8* pdec);
	void Encript1Time(uint_8* pdec, int cnt);
	uint_8* EncriptXmlFile(string path, uint_32& size);
};



void encryption::Encript1Time(uint_8* pdec, int cnt)
{
	uint_32 ttab = 0;
	uint_32 temp = 0;
	uint_32 temp1 = 0;
	uint_32 temp2 = 0;
	uint_32 temp3 = 0;
	uint_32 temp4 = 0;
	uint_32 tempx[4]{};
	uint_8* pdec1 = NULL;
	/*-----------------------------------------------------------*/
	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[0]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[13]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[10]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[7]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 1) * 16);
	temp ^= ttab;
	tempx[3] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[4]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[1]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[14]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[11]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 1) * 16 + 4);
	temp ^= ttab;
	tempx[2] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[8]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[5]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[2]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[15]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 1) * 16 + 4 * 2);
	temp ^= ttab;
	tempx[1] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[12]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[9]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[6]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[3]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 1) * 16 + 4 * 3);
	temp ^= ttab;
	tempx[0] = temp;
	/*-----------------------------------------------------------*/

	/*-----------------------------------------------------------*/
	pdec1 = (uint_8*)tempx;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec1[12]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec1[1]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec1[6]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec1[11]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 2) * 16 + 4 * 0);
	temp ^= ttab;
	*(uint_32*)pdec = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec1[8]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec1[13]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec1[2]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec1[7]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 2) * 16 + 4 * 1);
	temp ^= ttab;
	*(uint_32*)(pdec + 4) = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec1[4]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec1[9]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec1[14]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec1[3]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 2) * 16 + 4 * 2);
	temp ^= ttab;
	*(uint_32*)(pdec + 8) = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec1[0]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec1[5]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec1[10]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec1[15]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + (2 * cnt - 2) * 16 + 4 * 3);
	temp ^= ttab;
	*(uint_32*)(pdec + 12) = temp;
}



void encryption::Encript16ByteEmxl(uint_8* pdec)
{
	uint_32 tsrc = 0;
	uint_32 ttab = 0;
	uint_32 temp = 0;
	uint_32 temp1 = 0;
	uint_32 temp2 = 0;
	uint_32 temp3 = 0;
	uint_32 temp4 = 0;
	uint_32 tempx[4]{};
	uint_8* pdec1 = NULL;

	int i;

	for (i = 0;i < 4;i++)
	{
		tsrc = *(uint_32*)(pdec + 4 * i);
		ttab = *(uint_32*)(pUint_44 + 0xA * 16 + 4 * i);
		*(uint_32*)(pdec + 4 * i) = tsrc ^ ttab;
	}

	for (i = 5;i > 1;i--)
	{
		Encript1Time(pdec, i);
	}

	/*-----------------------------------------------------------*/
	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[0]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[13]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[10]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[7]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + 16);
	temp ^= ttab;
	tempx[3] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[4]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[1]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[14]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[11]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + 16 + 4);
	temp ^= ttab;
	tempx[2] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[8]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[5]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[2]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[15]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + 16 + 4 * 2);
	temp ^= ttab;
	tempx[1] = temp;

	temp1 = *(uint_32*)(pUint_256_1 + 4 * pdec[12]);
	temp2 = *(uint_32*)(pUint_256_2 + 4 * pdec[9]);
	temp = temp1 ^ temp2;
	temp3 = *(uint_32*)(pUint_256_3 + 4 * pdec[6]);
	temp ^= temp3;
	temp4 = *(uint_32*)(pUint_256_4 + 4 * pdec[3]);
	temp ^= temp4;
	ttab = *(uint_32*)(pUint_44 + 16 + 4 * 3);
	temp ^= ttab;
	tempx[0] = temp;
	/*-----------------------------------------------------------*/

	/*-----------------------------------------------------------*/
	pdec1 = (uint_8*)tempx;

	temp1 = *(uint_8*)(pUchar_256 + pdec1[12]);
	temp2 = *(uint_8*)(pUchar_256 + pdec1[1]);
	temp3 = *(uint_8*)(pUchar_256 + pdec1[6]);
	temp4 = *(uint_8*)(pUchar_256 + pdec1[11]);
	temp = temp4 * 0x1000000 + temp3 * 0x10000 + temp2 * 0x100 + temp1;
	ttab = *(uint_32*)(pUint_44 + 4 * 0);
	temp ^= ttab;
	*(uint_32*)pdec = temp;



	temp1 = *(uint_8*)(pUchar_256 + pdec1[8]);
	temp2 = *(uint_8*)(pUchar_256 + pdec1[13]);
	temp3 = *(uint_8*)(pUchar_256 + pdec1[2]);
	temp4 = *(uint_8*)(pUchar_256 + pdec1[7]);
	temp = temp4 * 0x1000000 + temp3 * 0x10000 + temp2 * 0x100 + temp1;
	ttab = *(uint_32*)(pUint_44 + 4 * 1);
	temp ^= ttab;
	*(uint_32*)(pdec + 4) = temp;

	temp1 = *(uint_8*)(pUchar_256 + pdec1[4]);
	temp2 = *(uint_8*)(pUchar_256 + pdec1[9]);
	temp3 = *(uint_8*)(pUchar_256 + pdec1[14]);
	temp4 = *(uint_8*)(pUchar_256 + pdec1[3]);
	temp = temp4 * 0x1000000 + temp3 * 0x10000 + temp2 * 0x100 + temp1;
	ttab = *(uint_32*)(pUint_44 + 4 * 2);
	temp ^= ttab;
	*(uint_32*)(pdec + 8) = temp;

	temp1 = *(uint_8*)(pUchar_256 + pdec1[0]);
	temp2 = *(uint_8*)(pUchar_256 + pdec1[5]);
	temp3 = *(uint_8*)(pUchar_256 + pdec1[10]);
	temp4 = *(uint_8*)(pUchar_256 + pdec1[15]);
	temp = temp4 * 0x1000000 + temp3 * 0x10000 + temp2 * 0x100 + temp1;
	ttab = *(uint_32*)(pUint_44 + 4 * 3);
	temp ^= ttab;
	*(uint_32*)(pdec + 12) = temp;

	/*-----------------------------------------------------------*/

}


uint_8* encryption::EncriptXmlFile(string path, uint_32& size)
{
	FILE* fp = fopen(path.c_str(), "rb");
	fseek(fp, 0, SEEK_END);
	uint_32 file_size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	uint_8* pdec = new uint_8[file_size];
	fread(pdec,1,file_size,fp);
	uint_8* presult = pdec;
	uint_32 i = 0;
	for (i = 0;i < file_size / 16;i++)
	{
		Encript16ByteEmxl(pdec);
		pdec += 16;
	}
	size = file_size;
	return presult;
}




int main()
{
	encryption xx;
	uint_32 size;
	uint_8* buffer=xx.EncriptXmlFile("C:\\Users\\Administrator\\Desktop\\zz\\diagnostic.exml", size);
	FILE* fp = fopen("C:\\Users\\Administrator\\Desktop\\zz\\diagnostic.xml", "wb");
	fwrite(buffer, 1, size, fp);
	return 0;
}