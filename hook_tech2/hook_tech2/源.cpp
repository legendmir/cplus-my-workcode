...
bool RemoteThreadInject(CString pName)
{
	...
	//2.申请内存,写入DLL路径
	int nLen = strlen("C:\\Users\\Administrator\\Desktop\\hook\\hookdll.dll") + 1;
	...
	//3.写入内存
	SIZE_T dwWrite = 0;
	if (!WriteProcessMemory(hProcess, pBuf, "C:\\Users\\Administrator\\Desktop\\hook\\hookdll.dll", nLen, &dwWrite))
	{
		printf("写入内存失败！\n");
		return false;
	}
...
int main()
{
	RemoteThreadInject("emulator.exe");
	system("pause\n");
	return 0;
}
