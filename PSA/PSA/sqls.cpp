...
c_sql::c_sql()
{
	...
	//¶¯×÷²âÊÔ
	xx.name = "GPC";
	xx.sql = "SELECT tamscn.SCNNAME, tam.TAMNAME, tam.TITLE, tam.TAMTYPE, tam.SHOWANIMATION, tam.HELPFILENAME, tam.TAMHELPTEXT, tam.STARTBYUSER, tam.STOPBYUSER, tam.PROPOSERESTART, tamscr.TAMSCRID, tamscr.MAINTEXT, tamscr.SCRHELPTEXT, tamscr.SCENARIOREF, tamscr.SCRTYPE, tamscr.SCRMINDURATION, tamscr.DEFAULTERRORTEXT FROM ECU ecu, I_ECUTAM iecutam,  TAM tam, TAMSCN tamscn, I_TAMSCR itamscr, TAMSCR tamscr WHERE itamscr.TAMSCRID = tamscr.TAMSCRID AND tam.TAMID = itamscr.TAMID AND iecutam.TAMID = tam.TAMID AND iecutam.TAMSCNID = tamscn.TAMSCNID AND ecu.ECUID = iecutam.ECUID AND ecu.ECUID = %s AND tam.TAMNAME = '%s' ";
	sql_map[st_action_test_scnnane] = xx;
	//
	xx.name = "GPC"; //3×é ecu_id ,scnnane
	xx.sql = "SELECT tamctrl.TAMCTRLID, tamctrl.CTRLNAME, tamctrl.CTRLTYPE, tamctrl.SENDSTATUSREQUESTAFTERSTOP, tamctrl.MINTIMEBEFORERESTART, tamctrl.MAXDURATION, tamctrl.SENDSTATUSREQUEST, tamctrl.CTRLSTATUSPARNAME, tamctrl.CTRLSTATUSPARLABEL,  tamserv.SERVNAME, tamserv.SERVROLE, tamserv.SERVICE, tamserv.SUBSERVICE, iservpar.ISERVPARVALUE, tamparam.PARNAME, tamparam.PARTYPE FROM ECU ecu, I_ECUTAM iecutam, TAMSCN tamscn, TAMCTRL tamctrl, I_CTRLSERV ictrlserv, TAMSERV tamserv, I_SERVPAR iservpar, TAMPARAM tamparam WHERE ecu.ECUID = iecutam.ECUID AND tamctrl.TAMCTRLID = iecutam.TAMCTRLID AND ictrlserv.TAMCTRLID = tamctrl.TAMCTRLID AND tamserv.TAMSERVID = ictrlserv.TAMSERVID AND iservpar.TAMSERVID = tamserv.TAMSERVID AND tamparam.TAMPARAMID = iservpar.TAMPARAMID AND tamscn.TAMSCNID = iecutam.TAMSCNID AND ecu.ECUID = %s AND tamscn.SCNNAME = '%s' UNION ALL SELECT tamctrl.TAMCTRLID, tamctrl.CTRLNAME, tamctrl.CTRLTYPE, tamctrl.SENDSTATUSREQUESTAFTERSTOP, tamctrl.MINTIMEBEFORERESTART, tamctrl.MAXDURATION, tamctrl.SENDSTATUSREQUEST, tamctrl.CTRLSTATUSPARNAME, tamctrl.CTRLSTATUSPARLABEL , CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(32)), CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(16)) FROM ECU ecu, I_ECUTAM iecutam, TAMSCN tamscn, TAMCTRL tamctrl WHERE ecu.ECUID = iecutam.ECUID AND tamctrl.TAMCTRLID = iecutam.TAMCTRLID AND tamscn.TAMSCNID = iecutam.TAMSCNID AND ecu.ECUID = %s AND tamscn.SCNNAME = '%s' AND NOT EXISTS (SELECT 1 FROM I_CTRLSERV ictrlserv WHERE ictrlserv.TAMCTRLID = tamctrl.TAMCTRLID) UNION ALL SELECT tamctrl.TAMCTRLID, tamctrl.CTRLNAME, tamctrl.CTRLTYPE, tamctrl.SENDSTATUSREQUESTAFTERSTOP, tamctrl.MINTIMEBEFORERESTART, tamctrl.MAXDURATION, tamctrl.SENDSTATUSREQUEST, tamctrl.CTRLSTATUSPARNAME, tamctrl.CTRLSTATUSPARLABEL ,  tamserv.SERVNAME, tamserv.SERVROLE, tamserv.SERVICE, tamserv.SUBSERVICE, CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(64)), CAST (NULL AS VARCHAR(16)) FROM ECU ecu, I_ECUTAM iecutam, TAMSCN tamscn, TAMCTRL tamctrl, I_CTRLSERV ictrlserv, TAMSERV tamserv WHERE ecu.ECUID = iecutam.ECUID AND tamctrl.TAMCTRLID = iecutam.TAMCTRLID AND ictrlserv.TAMCTRLID = tamctrl.TAMCTRLID AND tamserv.TAMSERVID = ictrlserv.TAMSERVID AND tamscn.TAMSCNID = iecutam.TAMSCNID AND ecu.ECUID = %s AND tamscn.SCNNAME = '%s' AND NOT EXISTS (SELECT 1 FROM I_SERVPAR iservpar WHERE iservpar.TAMSERVID = tamserv.TAMSERVID) ";
	sql_map[st_action_test_TAMCTRLID] = xx;
	//
	xx.name = "DSD";
	xx.sql = "SELECT s.SERID, s.SERDESCRIPTION, s.SERLNAME FROM SERVICE s inner join I_ECUSER i on s.serid = i.serid where i.ECUVEID =%s and (SERSNAME='%s')";
	sql_map[st_action_test_serid] = xx;
	//
	xx.name = "DSD";// veid,serid ,SUBSERVICE
	xx.sql = "SELECT s.SERID, s.SERSNAME, s.SERLNAME, s.SERDESCRIPTION, su.SERUNID, su.SERUNTYPE, su.SERUNDESCRIPTION, su.SERUNRECEIVECYCLES, su.SERUNSENDCYCLES, su.SERUNTIMECYCLES, sf.SERUNFRID, sf.SERUNFRTYID, sf.SERUNFRDESCRIPTION FROM SERVICE s inner join SERVUNIT su on s.SERID = su.SERID inner join SERVUNITFRAME sf on su.SERUNID = sf.SERUNID inner join I_ECUSER i on s.serid = i.serid where i.ECUVEID =%s and s.SERID = %s and sf.serunfrtyid = 0 and su.SERUNSNAME = '%s' ";
	sql_map[st_action_test_serunid] = xx;
	//
	xx.name = "DSD";//ser_unid
	xx.sql = "SELECT distinct sf.SERUNFRTYID, su.SERUNID , sf.SERUNFRDESCRIPTION, sf.SERUNFRID FROM SERVUNIT su inner join SERVUNITFRAME sf on su.SERUNID = sf.SERUNID where su.SERUNID = %s ";
	sql_map[st_action_test_serunfrid] = xx;
	//
	xx.name = "DSD";//unfrid
	xx.sql = "SELECT isp.SERUNFRID, isp.PARID, isp.ISPBYTEPOS, isp.ISPVALUE, isp.BLOID, pt.PARTYNAME, pt.PARTYID, p.PARSNAME, p.PARLNAME, p.PARDESCRIPTION, p.PARENCODING, p.PARTYPE, p.PARBASEENCODING, p.PARINTERNALENCODING, p.PARPHYSICALENCODING, p.PARCOMPUTMETHOD, dt.DATTYID, dt.DATTYNAME, ad.ADDABSOLUTENUMBER, ad.ADDBYTELENGTH, ad.ADDBYTEORDER, ad.ADDBITLENGTH, ad.ADDBITMASK, ad.ADDBITCONDENSED, ad.ADDMAXLENGTH, ad.ADDMINLENGTH, ad.ADDTERMINATION, ad.ADDBITPARAMREF, adt.ADDTYNAME, adt.ADDTYID, c.CRCSNAME, c.CRCLNAME, c.CRCDESCRIPTION, c.CRCFIRSTBYTE, c.CRCLASTBYTE, c.CRCPOLYNOM, c.CRCBYTELENGTH, c.CRCBLOCKNAME, bl.BLOSNAME, bl.BLODESCRIPTION, bl.BLOBYTEPOS, bl.BLOMINOCCURS, bl.BLOMAXOCCURS, bl.BLOBYTELENGTH, bl.ENDOFPDU, mp.MAPSNAME, mp.MAPLNAME, mp.MAPDESCRIPTION, mp.MAPLID, mp.MAPID, db.DYNSNAME, db.DYNBYTEPOS, db.DYNID, db.DYNBYTELENGTH FROM PARTYPE pt INNER JOIN I_SERPAR isp ON isp.PARTYID = pt.PARTYID LEFT OUTER JOIN (PARAM p  INNER JOIN DATATYPE dt ON p.DATTYID = dt.DATTYID) ON isp.PARID = p.PARID LEFT OUTER JOIN ADDDATA ad ON isp.PARID = ad.PARID LEFT OUTER JOIN ADDTYPE adt ON ad.ADDTYID = adt.ADDTYID LEFT OUTER JOIN BLOCK bl ON isp.BLOID = bl.BLOID LEFT OUTER JOIN DYNAMICBLOCK db ON isp.DYNID = db.DYNID LEFT OUTER JOIN CRC c ON isp.CRCID = c.CRCID LEFT OUTER JOIN MAPPED mp ON isp.MAPID = mp.MAPID WHERE (isp.SERUNFRID = %s) and ((isp.PARID is not null) OR (isp.CRCID is not null) OR (isp.MAPID is not null)) ";
	sql_map[st_action_test_req_info] = xx;
	
	xx.name = "DSD";//parid
	xx.sql = "SELECT distinct st.PARID, st.STATYID, st.STASNAME, st.STAVALUE, st.STAMINVALUE, st.STAMAXVALUE, st.STAMINVALUETYPE, st.STAMAXVALUETYPE, stt.STATYNAME, st.STALNAME FROM  STATES st, STATESTYPE stt WHERE stt.STATYID = st.STATYID and st.PARID in (%s) and st.STASNAME='%s'";
	sql_map[st_action_test_req_stvalue] = xx;
	//
	xx.name = "DSD";//unfrid
	xx.sql = "SELECT isp.SERUNFRID, isp.PARID, isp.ISPBYTEPOS, isp.ISPVALUE, isp.BLOID, pt.PARTYNAME, pt.PARTYID, p.PARSNAME, p.PARLNAME, p.PARDESCRIPTION, p.PARENCODING, p.PARTYPE, p.PARBASEENCODING, p.PARINTERNALENCODING, p.PARPHYSICALENCODING, p.PARCOMPUTMETHOD, dt.DATTYID, dt.DATTYNAME, ad.ADDABSOLUTENUMBER, ad.ADDBYTELENGTH, ad.ADDBYTEORDER, ad.ADDBITLENGTH, ad.ADDBITMASK, ad.ADDBITCONDENSED, ad.ADDMAXLENGTH, ad.ADDMINLENGTH, ad.ADDTERMINATION, ad.ADDBITPARAMREF, adt.ADDTYNAME, adt.ADDTYID, c.CRCSNAME, c.CRCLNAME, c.CRCDESCRIPTION, c.CRCFIRSTBYTE, c.CRCLASTBYTE, c.CRCPOLYNOM, c.CRCBYTELENGTH, c.CRCBLOCKNAME, bl.BLOSNAME, bl.BLODESCRIPTION, bl.BLOBYTEPOS, bl.BLOMINOCCURS, bl.BLOMAXOCCURS, bl.BLOBYTELENGTH, bl.ENDOFPDU, mp.MAPSNAME, mp.MAPLNAME, mp.MAPDESCRIPTION, mp.MAPLID, mp.MAPID, db.DYNSNAME, db.DYNBYTEPOS, db.DYNID, db.DYNBYTELENGTH FROM PARTYPE pt INNER JOIN I_SERPAR isp ON isp.PARTYID = pt.PARTYID LEFT OUTER JOIN (PARAM p  INNER JOIN DATATYPE dt ON p.DATTYID = dt.DATTYID) ON isp.PARID = p.PARID LEFT OUTER JOIN ADDDATA ad ON isp.PARID = ad.PARID LEFT OUTER JOIN ADDTYPE adt ON ad.ADDTYID = adt.ADDTYID LEFT OUTER JOIN BLOCK bl ON isp.BLOID = bl.BLOID LEFT OUTER JOIN DYNAMICBLOCK db ON isp.DYNID = db.DYNID LEFT OUTER JOIN CRC c ON isp.CRCID = c.CRCID LEFT OUTER JOIN MAPPED mp ON isp.MAPID = mp.MAPID WHERE (isp.SERUNFRID = %s) and ((isp.PARID is not null) OR (isp.CRCID is not null) OR (isp.MAPID is not null)) ";
	sql_map[st_action_test_rec_info] = xx;
	//
	xx.name = "DSD";//rec_info1
	xx.sql = "SELECT isp.SERUNFRID, isp.PARID, isp.ISPBYTEPOS, isp.ISPVALUE, isp.BLOID, pt.PARTYNAME, pt.PARTYID, p.PARSNAME, p.PARLNAME, p.PARDESCRIPTION, p.PARENCODING, p.PARTYPE, p.PARBASEENCODING, p.PARINTERNALENCODING, p.PARPHYSICALENCODING, p.PARCOMPUTMETHOD, dt.DATTYID, dt.DATTYNAME, ad.ADDABSOLUTENUMBER, ad.ADDBYTELENGTH, ad.ADDBYTEORDER, ad.ADDBITLENGTH, ad.ADDBITMASK, ad.ADDBITCONDENSED, ad.ADDMAXLENGTH, ad.ADDMINLENGTH, ad.ADDTERMINATION, ad.ADDBITPARAMREF, adt.ADDTYNAME, adt.ADDTYID, c.CRCSNAME, c.CRCLNAME, c.CRCDESCRIPTION, c.CRCFIRSTBYTE, c.CRCLASTBYTE, c.CRCPOLYNOM, c.CRCBYTELENGTH, c.CRCBLOCKNAME, bl.BLOSNAME, bl.BLODESCRIPTION, bl.BLOBYTEPOS, bl.BLOMINOCCURS, bl.BLOMAXOCCURS, bl.BLOBYTELENGTH, bl.ENDOFPDU, mp.MAPSNAME, mp.MAPLNAME, mp.MAPDESCRIPTION, mp.MAPLID, mp.MAPID, db.DYNSNAME, db.DYNBYTEPOS, db.DYNID, db.DYNBYTELENGTH FROM PARTYPE pt INNER JOIN I_SERPAR isp ON isp.PARTYID = pt.PARTYID LEFT OUTER JOIN (PARAM p  INNER JOIN DATATYPE dt ON p.DATTYID = dt.DATTYID) ON isp.PARID = p.PARID LEFT OUTER JOIN ADDDATA ad ON isp.PARID = ad.PARID LEFT OUTER JOIN ADDTYPE adt ON ad.ADDTYID = adt.ADDTYID LEFT OUTER JOIN BLOCK bl ON isp.BLOID = bl.BLOID LEFT OUTER JOIN DYNAMICBLOCK db ON isp.DYNID = db.DYNID LEFT OUTER JOIN CRC c ON isp.CRCID = c.CRCID LEFT OUTER JOIN MAPPED mp ON isp.MAPID = mp.MAPID WHERE (isp.SERUNFRID = %s) AND (isp.PARID is null) AND (isp.BLOID is not null) AND (isp.PARTYID = 7) ";
	sql_map[st_action_test_rec_info1] = xx;
	//

	xx.name = "DSD";//parid
	xx.sql = "SELECT distinct st.PARID, st.STATYID, st.STASNAME, st.STAVALUE, st.STAMINVALUE, st.STAMAXVALUE, st.STAMINVALUETYPE, st.STAMAXVALUETYPE, stt.STATYNAME, st.STALNAME FROM  STATES st, STATESTYPE stt WHERE stt.STATYID = st.STATYID and st.PARID in (%s) ";
	sql_map[st_action_test_rec_stvalue] = xx;
	//
	xx.name = "GPC";//TAMCTRLID
	xx.sql = "SELECT tamstatusdiscval.TAMSTATUSDISCVALNAME, tamstatusdiscval.TAMSTATUSDISCVALTYPE, tamstatusdiscval.TAMSTATUSDISCVALLABEL  FROM TAMCTRL tamctrl, I_TAMCTRLSTATUS itamctrlstatus, TAMSTATUSDISCVAL tamstatusdiscval WHERE itamctrlstatus.TAMCTRLID = tamctrl.TAMCTRLID AND tamstatusdiscval.TAMSTATUSDISCVALID = itamctrlstatus.TAMSTATUSDISCVALID AND tamctrl.TAMCTRLID = %s ";
	sql_map[st_action_test_TAMSTATUSDISCVALNAME] = xx;
